...
describe('senseng toy templates (senseng-candy, senseng-wonder, senseng-arcade, senseng-nature, senseng-minimal)', () => {
  ...
  it('safely renders all pages (including detail with empty or missing productId) across templates', () => {
    const templates: Draft['template'][] = [
      'senseng-clean',
      'senseng-video',
      'senseng-candy',
      'senseng-wonder',
    ...
    for (const t of templates) {
      ...
      // 1. Normal draft with empty productId on detail
      const detailHtml = renderSite(d, { ...opts, page: 'detail', productId: '' });
      expect(detailHtml).toContain('<!doctype html>');
      expect(detailHtml).not.toContain('products//index.html');

      // 2. Normal draft with undefined productId on detail
      const detailNoIdHtml = renderSite(d, { ...opts, page: 'detail', productId: undefined });
      expect(detailNoIdHtml).toContain('<!doctype html>');

      // 3. Draft with no products and no primaryProductId
      const emptyDraft = { ...d, products: [], primaryProductId: '' };
      for (const page of ['home', 'catalog', 'detail', 'about', 'contact']) {
        const pageHtml = renderSite(emptyDraft, { ...opts, page, productId: '' });
