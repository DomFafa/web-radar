...
describe('versioned internal template documents', () => {
  it('contains exactly one independent document for each current template', () => {
    ...
    expect(new Set(templateGuides.map((g) => g.visualSystem.artDirection)).size).toBe(29);
    expect(new Set(templateGuides.map((g) => g.visualSystem.composition)).size).toBe(29);
  });
