...
describe('versioned internal template documents', () => {
  it('contains exactly one independent document for each current template', () => {
    ...
    expect(new Set(templateGuides.map((g) => g.visualSystem.artDirection)).size).toBe(39);
    expect(new Set(templateGuides.map((g) => g.visualSystem.composition)).size).toBe(39);
  });
