...
function pageDesignParts(
    ...
    products: draft.products.map((product) => ({
      ...
      ...(page === 'detail' && product.id === draft.primaryProductId
        ? {
            ...
            ...(language === 'en' ? {tagline:product.tagline,sellingPoints:product.sellingPoints,applications:product.applications} : {}),
            description:
  ...
  if (page === 'detail') for (const image of primary?.gallery ?? []) {
    if (seenAssets.has(image.assetId)) continue;
    seenAssets.add(image.assetId);
    referenceIndexByAsset.set(image.assetId, referenceRoles.length);
    referenceRoles.push(`selected ${image.kind} gallery image for ${primary!.id}; preserve the supplied view`);
  }
