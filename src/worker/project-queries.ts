...
export async function listProjectSummaries(
  ...
  const rows = await db
    .prepare(
      `SELECT json_object('id',id,'name',json_extract(data,'$.name'),'companyName',json_extract(data,'$.draft.company.name'),'productCount',json_array_length(data,'$.draft.products'),'template',json_extract(data,'$.draft.template'),'coverAssetId',coalesce(json_extract(data,'$.draft.banner.assetId'),json_extract(data,'$.draft.siteDesign.pages.home.imageAssetId'),json_extract(data,'$.draft.posterAssetId'),(SELECT json_extract(value,'$.imageAssetId') FROM json_each(projects.data,'$.draft.products') WHERE json_extract(value,'$.imageAssetId') IS NOT NULL LIMIT 1)),'updatedAt',json_extract(data,'$.updatedAt'),'createdAt',json_extract(data,'$.createdAt'),'offline',json_extract(data,'$.offline'),'publishedReleaseId',json_extract(data,'$.publishedReleaseId')) AS summary FROM projects WHERE ${where}${status !== 'all' ? ` AND (${state})=?` : ''} ORDER BY json_extract(data,'$.updatedAt') DESC, json_extract(data,'$.createdAt') DESC, id DESC LIMIT ? OFFSET ?`,
    )
