...
export function typedMaterialsDemoDraft(profile:MaterialsTemplateContract,lang:Language){
  ...
  draft.company={...draft.company,name:'Example Brand',description:'Explore this illustrated example collection and discuss product details with our team.',email:'demo@example.invalid',contactName:'Example team'};
  const asset=(role:string,index:number)=>role==='collection'&&['fintech-platform','corpox-consulting'].includes(profile.templateId)?`/templates/materials-demo/dark-collection-${index}.svg`:`/templates/juno-display-demo/${role}-${index}.svg`;
  draft.products=Array.from({length:4},(_,i)=>({id:`example-${i}`,name:`Example toy ${i+1}`,description:'Illustrated example product. Request confirmed specifications and available options.',material:'',dimensions:'',imageAssetId:asset('front',i),gallery:[{assetId:asset('front',i),caption:'Illustrated front view',sourceImageId:`front-${i}`,kind:'original' as const},{assetId:asset('packaging',i),caption:'Illustrated packaging example',sourceImageId:`packaging-${i}`,kind:'detail' as const}]}));
  ...
  const textBindings=profile.textSlots.flatMap(slot=>draft.languages.map(locale=>{
    ...
    const text=core[slot.id]||(slot.id.endsWith('seo-title')?'Example Brand | Toy collection':slot.id.endsWith('seo-description')?copy.subtitle:interfaceCopy.test(example)?example:/^[\d.,%+\s$KkMm]+$/.test(example)?'—':slot.maxCodePoints>180?copy.subtitle:'Explore the collection');
  ...
  const imageBindings:AppliedMaterials['imageBindings']=profile.imageSlots.flatMap(slot=>{
    ...
    return targets.map(target=>{
      ...
      return{slotId:slot.id,...target,assetId,fit:slot.fit,focalPoint:{x:.5,y:.5},alt:Object.fromEntries(draft.languages.map(l=>[l,`Illustrated example: ${slot.role||'original product image'}`])),...(slot.role?{role:slot.role,depictedProductIds:slot.role==='collection'?ids:target.productId?[target.productId]:[]}:{}),...(slot.role==='packaging'?{evidenceAssetIds:[asset('packaging',index)]}:{})};
    });
  });
  draft.materials={templateId:profile.templateId,contractRevision:profile.contractRevision,displaySelection:selection,visual:{palette:{primary:'#236BB0',secondary:'#E9AB4D',background:'#FAFBFF',surface:'#FFFFFF',text:'#20263E',mutedText:'#626B7E'},backgroundStyle:'plain',imageTreatment:'natural',compositionSummary:'Labelled illustrated examples for this template.'},textBindings,imageBindings,omittedSectionIds:profile.optionalSections.map(s=>s.id)};
