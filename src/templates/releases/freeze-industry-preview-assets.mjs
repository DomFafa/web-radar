...
for (const name of ['industry-preview-20260922.ts', 'industry-preview-20260922.json', 'industry-assets-20260922.json'])
...
const snapshot = '// Generated from reviewed baseline; never replace a published runtime.\nexport const frozenIndustryPreviewRuntime: string = ' + JSON.stringify(projectPreviewRuntime) + ';\n';

const renderer = readFileSync(resolve(dir, 'industry-20260922.mjs'), 'utf8');
...
writeFileSync(resolve(dir, 'industry-preview-20260922.ts'), snapshot);
writeFileSync(resolve(dir, 'industry-preview-20260922.json'), JSON.stringify({ sourceCommit, runtimeSha256: digest(projectPreviewRuntime), runtimeBytes: Buffer.byteLength(projectPreviewRuntime), snapshotSha256: digest(snapshot) }, null, 2) + '\n');
writeFileSync(resolve(dir, 'industry-assets-20260922.json'), JSON.stringify({ sourceCommit, rendererSha256: digest(renderer), assets, externalCssUrls: [...externalCssUrls].sort() }, null, 2) + '\n');
