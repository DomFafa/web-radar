// Immutable repair for the eight PR #8 templates. Publish a new revision for later edits.
...
// src/templates/themes/types.ts
function safeUrl(value, blob = false) {
  try {
    if (value.startsWith("/") && !value.startsWith("//") && !value.includes("\\")) return value;
    const u = new URL(value);
    if (u.username || u.password) return "";
    if (u.protocol === "https:" || u.protocol === "http:" && ["localhost", "127.0.0.1", "[::1]"].includes(u.hostname) || blob && u.protocol === "blob:")
      return u.href;
  } catch {
  }
  return "";
...
// src/templates/materials-outreach-demo-repair.ts
import { repairMaterialsDemo } from "./demo-20260923.mjs";
import { getMaterialsTemplate } from "./outreach-20260923.mjs";
function getOutreachDemoContract(id) {
...
__name(getOutreachDemoContract, "getOutreachDemoContract");
function repairOutreachMaterials(html, draft, options) {
  const document = parse(repairMaterialsDemo(html, draft, options));
  const visit = /* @__PURE__ */ __name((node, inHeader = false) => {
    if ("tagName" in node) {
...
__name(repairOutreachMaterials, "repairOutreachMaterials");
export {
  getOutreachDemoContract,
  repairOutreachMaterials
};
