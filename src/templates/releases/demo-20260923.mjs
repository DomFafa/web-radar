// Immutable materials.6 repair layer; never replace a published renderer.
// Authoring source: src/templates/materials-demo-repair.ts. Bundles its parse5 dependency.
...
// src/templates/materials-demo-repair.ts
var attr = (node, name) => node.attrs.find((a) => a.name === name)?.value;
function repairMaterialsDemo(html, draft, options) {
  ...
  const visit = (node, brandHeader = false, industryFooter = false) => {
    if ("tagName" in node) {
      ...
      if (node.tagName === "script" && !attr(node, "src") && (!attr(node, "type") || attr(node, "type") === "text/javascript")) {
        for (const text of node.childNodes) if (text.nodeName === "#text" && "value" in text) {
          const names = [...new Set([...text.value.matchAll(/\b(__name\d*)\s*\(/g)].map((match) => match[1]))].filter((name) => !new RegExp(`\\b(?:var|let|const|function)\\s+${name}\\b`).test(text.value));
