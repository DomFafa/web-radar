/**
 * Frozen confirmed-materials renderer from f71e496.
 * The 2026-09-17 Senseng contracts use positional text slots, so changing this
 * markup would reinterpret accepted material bindings. New templates must use
 * their own versioned renderer; standalone pages use themes/senseng.ts.
 */
import type { Product } from '../shared/model';
import { esc, safeUrl, type ThemeContext } from './themes/types';

const SENSENG_DEFAULT_PRODUCTS = [
  {
    name: 'Kids Squishy',
...
export function renderLegacySensengPage(ctx: ThemeContext, isVideoFullscreen = false, materialsMode=Boolean(ctx.draft.materials)): string {
