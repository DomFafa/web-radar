...
- 「资料与产品」与「预览与发布」均可管理多组页面 Banner，覆盖模板、以图生站及自定义生成三个分支。可指定首页、产品列表页、关于页、联系页与自定义独立页面；同组可复用于多个页面。商品详情页保持原设计，旧的详情页分配在保存时清理。旧版首页配置自动兼容。
...
- 按基本信息、联系与询盘、公司介绍、可展开的补充实力资料分组。
- 仅公司 / 品牌名称和有效联系邮箱为必填，前后端准备状态保持一致。联系人可留空，网站不展示空联系人标题或示例姓名。
- 明确邮箱的公开展示与通知用途；电话和 WhatsApp 分开填写并提示国家区号。
- 品牌一句话介绍与公司简介各有用途；资质、经营背景、定制和交付条件集中在选填区域，不填不会阻断制作。沿用已有数据字段，原有资料保留。
...
- `npm run check`：384 项测试、类型检查、生产构建通过。
