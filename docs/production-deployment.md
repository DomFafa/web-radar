...
- 公司 Logo 旁新增独立 Favicon 上传/移除，支持 PNG、JPEG、WebP、ICO；桌面并排、手机堆叠。ICO 检查文件头，图标资产仍按项目与发布版本授权。
- 模板、生成静态页面和旧版克隆页面在预览/发布时统一注入图标；替换原有 icon 链接。更换图标不清除已确认方案或页面设计，不需要重新调用模型；保存并发布更新即可。
- 类型检查、328 项测试、构建通过。真实 Chrome 验证 ICO 上传、保存后刷新、桌面/移动布局、预览、匿名公开图标和移除；本地验收项目已清理。无付费模型调用或数据库迁移。
- 线上版本：`ae111cf1-be0e-42d9-91d1-865afa72e2ec`；上一版本：`990c41b6-dad8-4b55-a482-fd0f7eec86ec`。构建资源：`index-CB7TuXBU.js` / `index-p6t9ztft.css`。


## 2026-09-17 本地产品套图联动版本上线

按用户要求将本地 `4f025cd` 加未提交的产品套图/网站文案联动改动部署到正式环境。Worker 版本 `c65b3487-1cb9-4a64-8fc2-7b593cf5eda3`，部署 `9c8fc3a5-e6c8-4651-876b-379945e488fc`，100% 流量；可回退版本 `ae111cf1-be0e-42d9-91d1-865afa72e2ec`。

- 使用 `wrangler deploy --keep-vars --env ''`。30 项绑定、线上配置及运行时逐项保持一致；无待执行 D1 迁移，发布前无进行中的 Worker 任务或 Python 构建任务。
- 下载线上 Worker 与本地 dry-run 包 SHA-256 完全一致；线上 HTML、JS、CSS 与本地构建逐字节一致。健康接口 200、`testMode:false`。前端为 `index-DeT_r0L_.js` / `index-DagSt_t8.css`。
- 按指定的本地版本发布，线上此前单独增加、尚未进入本地源码的 Favicon 上传入口不包含在本次版本中。
- 独立构建服务当前为 `20260915-product-sets-v1`，11 个 Python 运行模块、依赖声明及锁文件已经与本地一致，所以未更新或重启服务。鉴权探测正常，Product Radar 服务保持 active。
- `npm run check` 通过：类型检查、332 项测试、构建；Python 536 项测试通过，只有一项 Starlette/anyio 弃用警告。
- 使用当前正式账号从 Product Radar「已有网站」进入内嵌 Web Radar，登录衔接成功并读取 3 个现有项目。打开已有草稿的导入选择框，成功读取 8 个获准产品。没有提交导入、生成或客户网站发布，未改变账号范围；这些后续业务动作仍需实际试用验收。
- 部署记录、源文件哈希、配置快照和核验证据位于 `artifacts/deploy-local-20260917/`（Git 忽略；私有配置快照不可提交）。本次未创建提交或推送。

## 2026-09-17 三步流程入口修复

- 原因：创建接口只保存 `clone` 模式，忽略 `template` / `custom`；Product Radar 新项目交接也未写入模式。缺失模式时，导航沿用五步流程，而标题和部分操作显示三步。
- 新建项目保存明确选择的模式；Product Radar 交接默认进入三步模板流程。旧项目未指定模式时继续沿用五步定制，标题、模式卡片和操作保持一致；项目列表按实际流程显示继续入口。
- Worker 版本 `e31cd3b6-fb7d-4f60-abe1-c40ca950aea2`，100% 流量，可回退版本 `c65b3487-1cb9-4a64-8fc2-7b593cf5eda3`。使用 `--keep-vars`，30 项绑定及运行时保持一致；无迁移或 Python 服务改动。
- 新回归先复现模板、定制及交接默认模式三项保存失败，修复后 `npm run check` 的类型检查、337 项测试和构建通过。交接测试同时覆盖产品图片组与来源资料保留、重复交接去重，以及旧五步流程兼容。
- 正式账号的 `Imported website` 草稿通过现有界面补设模板模式，V1→V2；逐字段核对仅模式及正常版本/时间发生变化，5 个产品和全部资料保留。刷新后仍为三步，成功打开含 10 套模板的第二步。未生成或发布客户网站。
- 正式浏览器重新打开 `111` 也成功进入三步模板页面，项目数据未改动。下载的线上 Worker、HTML、JS、CSS 与本地构建一致，健康检查正常。验证材料位于 `artifacts/workflow-entry-20260917/`；本次未提交或推送。

## 2026-09-18 Product Radar 已确认资料交接

- 00:16:51（Asia/Shanghai）部署 Worker `822b147c-148e-42f1-b7ff-88a652a0850b`，100% 流量；回滚版本 `ab5914b1-60df-4ee6-806f-74a4f7f87f63` 保留。仅为 `vc.ddom@gmail.com` 开放新资料交接，客户网站仍须明确发布。
- 从隔离候选 `artifacts/materials-release-20260917/scoped-source` 部署。基线 `e630c669` 打包代码与部署前生产代码一致，仅依赖目录的路径注释不同。原工作区的旧套图导入、旧 AI 提示词和 Python 服务改动未随本次发布；新 gallery/copy 渲染仅在已接收的材料项目启用。
- 候选源码 SHA-256 `17ad0f917e79d7de7027dcf5193928ebfb9fcb7698b7c3cca8aced44fe6384bd`，Worker `aeac6832bdd84b4100ba125ee1c33d6f4e93e9d51e156b6f6d9eaa9f3440b8a8`。部署后下载的 Worker 与冻结包逐字节一致；666 个静态文件有完整清单，旧路径均保留，包括上一版前端 JS。
- 30 项原生产非静态绑定逐字段保持一致，无数据库迁移、Python 服务更新、账号范围扩大或客户网站发布。正式健康接口 200、`testMode:false`，未登录接口 401；正式账号读取 Juno、Senseng clean/video 新材料合同均为 200，错误密钥 401，缺失身份 400。11 项代表性线上静态文件与冻结清单的哈希一致。
- 隔离候选的类型检查、40 文件 / 467 项测试通过；最后桌面留白修正后重新执行 3 文件 / 18 项针对性测试、构建与打包检查。真实本地 A/B 交接分别接收 19/19、52/52 资产，均为未发布 V1、10 产品，原图保持第一张，Web Radar 未创建生成任务。
- A/B 正常登录后直接进入第三步预览，已核验桌面与手机首屏。手机完整显示主体；桌面询盘按钮完整位于 650px 首屏内，两套实际点击均进入联系页。已确认内容和哈希未因预览改变。
- 冻结清单、线上检查、有效首屏截图与范围说明位于 `artifacts/materials-release-20260917/`。Product Radar 配套版本由其发布协调任务切换并验证生产账号边界。本次未提交或推送 Git；整个原工作区仍包含未部署改动，不能将其视为生产版本的精确副本。

## 2026-09-18 新材料 Juno 首页空白修复

- 01:21:50（Asia/Shanghai）部署 Worker `8fb1d423-6ae9-4323-8614-c53bbf4be303`，100% 流量；回滚版本 `822b147c-148e-42f1-b7ff-88a652a0850b`。线上下载包与候选 SHA-256 `ae3b93bd9076d2d740755e42c7f911790d7320ef4318925cff676ca9f6fb2b11` 完全一致。
- 只在新 materials Juno 最终渲染移除无支持素材的旧宣传区块 `6f48de11`，以及省略新闻、评价、品牌区块后残留的四段纯 spacer。保留正常段落间距、旧项目行为和全部素材槽位；三模板合同及 revision 不变。
